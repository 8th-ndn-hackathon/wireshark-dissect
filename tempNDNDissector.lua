...
function getUriFromNameComponent(b)
   ...
   --print("Heres the uri from component:",s)
   return s
end
...
function getUriFromName(b)
   ...
   --print("getUriFromName")
   --for key,value in pairs(table.concat(components,"/")) do print(key,value) end
...
function getGenericBlockInfo(block)
   ...
   print("In generic block", name .. ", Type: " .. block.type .. ", Length: " .. block.length)
...
function addSummary(block)
   ...
   for k, subblock in pairs(block.elements) do
      if (subblock.value ~= nil) then
         ...
         if (info ~= nil) then
            --Workaround necessary because both ForwardingHint and LinkPreference have the same TLV-TYPE number.
...
function checkDataOrder(subBlocks)


end

function checkInterestOrder(subBlocks)
   max = 0
   local ORDERED_INTEREST_LIST = {[7] = 1, [33] =2, [18] = 3, [30] = 4, [10] = 5, [12] = 6, [34] = 7, [35] = 8}
   for k, subBlock in pair(subBlocks)do 
      if (ORDERED_INTEREST_LIST[subBlock.type] > max) then
        --
      else
         max = ORDERED_INTEREST_LIST[subBlock.type]
   end
end

function checkLOOrder(subBlocks)

end

--Let's have a function here for interests/data/lp order checking
function getSubBlocks(block)
   ...
   inOrder = false
   if(block.type == 5) then
      inOrder = checkInterestOrder
   else if(block.type == 6) then
      inOrder = checkDataOrder
   else
      inOrder = checkLOOrder
   end
