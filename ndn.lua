...
local NAME_COMPONENTS = {
   [0]   = {name = "Invalid"                         , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [1]   = {name = "ImplicitSha256DigestComponent"   , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [2]   = {name = "ParametersSha256DigestComponent" , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [8]   = {name = "GenericNameComponent"            , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [32]  = {name = "KeywordNameComponent"            , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [33]  = {name = "SegmentNameComponent"            , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [34]  = {name = "ByteOffsetNameComponent"         , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [35]  = {name = "VersionNameComponent"            , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [36]  = {name = "TimestampNameComponent"          , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
   [37]  = {name = "SequenceNumNameComponent"        , field = ProtoField.string("ndn.namecomponent", "NameComponent") , value = getUriFromNameComponent},
}
...
function addInfo(block, root) -- may be add additional context later
   ...
   if (info.value == nil) then
      ...
      if (block.type == 8) then
      end
     
      block.value = info.value(block)

      if (info.field ~= nil) then
         treeInfo = root:add(info.name, block.tvb(block.offset, block.size), block.value)
      else
...
function addNameComponentInfo(block, root) -- may be add additional context later
   local info = NAME_COMPONENTS[block.type]
         ...
         if (block.elements ~= nil) then
            for i, subBlock in pairs(block.elements) do
               ...
               if (block.type == 7) then
                  addNameComponentInfo(subBlock, subtree)
               else
